...
def show_menu
    loop do
        CLI::UI::Prompt.ask('OPÇÕES:') do |handler|
          handler.option('Adicionar contato') { |selection| selection }   #Colocar aqui as funções correspondentes
          handler.option('Alterar contato')   { |selection| selection }   #Colocar aqui as funções correspondentes
